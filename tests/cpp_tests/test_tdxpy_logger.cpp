...
TEST_F(TdxpyLoggerTest, Log_LevelFiltering) {
    // 测试DEBUG级别：应该记录所有级别的日志
    bool initResult = tdxpy::Logger::getInstance().initialize(testLogFile, tdxpy::LogDebug);
    ASSERT_TRUE(initResult);
    
    TDXPY_LOG_TRACE("Trace message at DEBUG level");
    TDXPY_LOG_DEBUG("Debug message at DEBUG level");
    TDXPY_LOG_INFO("Info message at DEBUG level");
...
TEST_F(TdxpyLoggerTest, Log_SpecialCharactersAndUnicode) {
    ...
    // 验证包含部分可识别内容
    EXPECT_NE(content.find("Unicode测试"), std::string::npos);
}
